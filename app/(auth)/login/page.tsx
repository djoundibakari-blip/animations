...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex" style={{ background: "#08080e" }}>
      ...
      {/* ══════════════════════════════════════════════
          PANNEAU GAUCHE — Hero festivités slideshow
      ══════════════════════════════════════════════ */}
          ...
          <div>
            <div className="auth-slide-up" style={{ animationDelay: "0.18s" }}>
              ...
              <p className="text-sm leading-relaxed max-w-[270px] mb-8" style={{ color: "rgba(255,255,255,0.4)" }}>
                Herald collecte, filtre et livre les meilleurs événements
                culturels de votre ville directement dans votre boîte mail.
              </p>
            </div>

            {/* Slide indicators + event name */}
      ...
      <div className="flex-1 flex items-center justify-center bg-white px-10 sm:px-16 py-12 relative overflow-hidden">
        ...
        <div className="w-full max-w-[320px] relative">
          ...
          {/* Header */}
          <div className="mb-10 auth-slide-up" style={{ animationDelay: "0.12s" }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {/* Email */}
            <div className="mb-6 auth-slide-up" style={{ animationDelay: "0.22s" }}>
            ...
            {/* Password */}
            <div className="mb-8 auth-slide-up" style={{ animationDelay: "0.3s" }}>
              <label className="block text-[10px] font-black tracking-[0.3em] uppercase mb-2.5" style={{ color: "#9ca3af" }}>
                Mot de passe
              </label>
              <div
            ...
            {/* Submit */}
            <div className="auth-slide-up" style={{ animationDelay: "0.37s" }}>
              <button
                type="submit"
                disabled={loading}
                className="w-full text-white py-4 px-6 flex items-center justify-between font-black text-[11px] uppercase tracking-[0.28em] transition-all duration-300 group disabled:opacity-60 disabled:cursor-not-allowed"
                style={{
                  background: loading
                    ? "#1f2937"
                ...
                onMouseEnter={(e) => {
                  if (!loading) (e.currentTarget as HTMLButtonElement).style.background = "linear-gradient(135deg, #6d28d9 0%, #4f46e5 100%)";
                }}
                onMouseLeave={(e) => {
                  if (!loading) (e.currentTarget as HTMLButtonElement).style.background = "linear-gradient(135deg, #1f2937 0%, #111827 100%)";
                }}
              >
          ...
          <div
            ...
            style={{ borderTop: "1px solid #f3f4f6", animationDelay: "0.45s" }}
          >
            <p className="text-xs text-gray-400">Pas encore de compte ?</p>
            <Link
              href="/register"
              className="text-xs font-black uppercase tracking-widest text-gray-900 flex items-center gap-1.5 group transition-colors duration-200"
          ...
          {/* Security badge */}
          <div
            className="mt-8 flex items-center gap-2 text-[10px] uppercase tracking-widest auth-fade-in"
            style={{ color: "#d1d5db", animationDelay: "0.58s" }}
          >
            <ShieldIcon />
            Connexion chiffrée · Conforme RGPD
          </div>
        </div>
      </div>
    </div>
